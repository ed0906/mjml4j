...
import ifusion.mjml.exception.MJMLBadRequestException;
import ifusion.mjml.exception.MJMLForbiddenException;
import ifusion.mjml.exception.MJMLInternalServerException;
import ifusion.mjml.exception.MJMLUnauthorizedException;
...
public class MJMLClientRenderTest extends MJMLClientHTTPTestBase {
    ...
    public void validRequestWithSuccessfulResponse() {
        ...
        givenThat(post(urlPathEqualTo("/" + VERSION + "/render"))
                ...
                .willReturn(
                        aResponse()
                        .withBody(fromFile("/ifusion/mjml/render-response_success.json"))
    ...
    public void unauthorizedResponse() {
        ...
        givenThat(post(urlPathEqualTo("/" + VERSION + "/render"))
                .willReturn(
                        aResponse()
                                .withBody(fromFile("/ifusion/mjml/render-response_unsuccessful.json"))
                                .withHeader("Content-Type", "application/json")
                                .withStatus(401)
                )
        );
